  ...
  const fetchData = async (ticker, period, interval = '1d') => {
    ...
    try {
      ...
      if (!indicatorsResult.success) {
        throw new Error(indicatorsResult.error || 'Failed to add technical indicators');
      }
      
      // Fetch sentiment analysis
      const sentimentResult = await fetchSentimentAnalysis(ticker);
      if (!sentimentResult.success) {
        throw new Error(sentimentResult.error || 'Failed to fetch sentiment data');
      }
